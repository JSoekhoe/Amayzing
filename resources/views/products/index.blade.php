<x-app-layout>
    ...
    <section class="py-10 max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <form method="GET" action="{{ route('products.index') }}"
              class="flex justify-center space-x-10 border-b-4 border-gray-400 pb-4 mb-6">
        ...
        @if(session('error'))
            <div class="max-w-7xl mx-auto mb-4 p-4 bg-red-100 border border-green-400 text-red-800 rounded-xl">
                {{ session('error') }}
            </div>
        @endif
        ...
        {{-- Bezorginformatie + postcodecheck --}}
        @if ($deliveryMethod === 'bezorgen')
            <div class="flex flex-col lg:flex-row max-w-6xl mx-auto gap-6 mb-6">
                ...
                <aside class="w-full lg:w-2/3 bg-gray-100 rounded-3xl shadow-sm p-6 text-gray-900">
                    ...
                    {{-- Leverschema’s (weken apart, altijd zichtbaar) --}}
                    <div class="space-y-6 text-gray-800">
                        ...
                        <div class="bg-white border border-gray-300 rounded-xl p-4 shadow-sm">
                            ...
                            @else
                                <p class="text-sm text-gray-500 italic">
                                    Geen geplande bezorgmomenten deze week.
                                </p>
                            @endif
                        </div>
                        ...
                        <div class="bg-white border border-gray-300 rounded-xl p-4 shadow-sm">
                            ...
                            @else
                                <p class="text-sm text-gray-500 italic">
                                    Geen geplande bezorgmomenten volgende week.
                                </p>
                            @endif
                        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            @forelse ($products as $product)
                @php
                    $availableStock = ($deliveryMethod === 'afhalen' || !$deliveryMethod)
                        ? $product->pickup_stock
                        : $product->delivery_stock;
                @endphp
                ...
                <article class="bg-white rounded-3xl shadow-lg p-6 flex flex-col justify-between hover:shadow-xl transition">
                    ...
                    @if ($availableStock > 0)
                        <form action="{{ route('cart.add', $product->id) }}" method="POST" class="mt-auto space-y-4">
                            @csrf
                            <input type="hidden" name="type" value="{{ $deliveryMethod ?? 'afhalen' }}">
                    ...
                    @else
                        <p class="text-red-600 font-semibold mt-4 text-center">
                            Niet beschikbaar voor {{ ($deliveryMethod === 'bezorgen') ? 'bezorgen' : 'afhalen' }}.
                        </p>
                    @endif
                </article>
            @empty
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (!postcodeInput || !straatnaamInput) return;
